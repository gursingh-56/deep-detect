...
import warnings
# Suppress deprecation warning from face_recognition_models
...
def get_accurate_model(sequence_length):
    ...
    if len(sequence_model) > 1:
        ...
        # Fallback: use the first available model if no exact match found
        print(f"No model found for sequence length {sequence_length}. Using first available model.")
        if model_name:
            final_model = os.path.join(settings.PROJECT_DIR, "models", model_name[0])
            print(f"Using model: {model_name[0]}")
        else:
...
ALLOWED_VIDEO_EXTENSIONS = set(['mp4','mov','gif','webm','avi','3gp','wmv','flv','mkv'])
    ...
    try:
        if request.method == "GET":
            ...
            if 'sequence_length' in request.session:
                ...
                sequence_length = 100  # Default fallback
        ...
        if not path_to_model or not os.path.exists(path_to_model):
            error_msg = f"No model found for sequence length {sequence_length}. "
            error_msg += "Please ensure a model file exists in the models directory."
            print(error_msg)
            return render(request, 'cuda_full.html', {'error': error_msg})
        ...
        for i in range(sequence_length):
            ...
            try:
                frame = frames[i]
                print(f"Processing frame {i+1}/{min(sequence_length, len(frames))}")
                ...
                print(f"  Detecting faces in frame {i+1}...")
                try:
                    face_locations = face_recognition.face_locations(rgb_frame, model='hog')  # Use 'hog' for faster processing
                    print(f"  Found {len(face_locations)} face(s) in frame {i+1}")
                except Exception as face_error:
                    print(f"  Error in face detection for frame {i+1}: {face_error}")
                    continue
                
                if len(face_locations) == 0:
                    print(f"  No faces found in frame {i+1}, skipping...")
                    continue
                ...
                print(f"  Saved cropped face for frame {i+1}")
            except Exception as frame_error:
                print(f"Error processing frame {i+1}: {frame_error}")
                import traceback
                traceback.print_exc()
                continue
        ...
        try:
            ...
            return render(request, 'cuda_full.html', error_context)
    except Exception as e:
        print(f"Fatal error in predict_page: {e}")
        import traceback
        traceback.print_exc()
        error_context = {
